...
void sha256_transform(std::vector<uint32_t>& state, const std::vector<uint8_t>& block) {
    // SHA-256 constants
...
std::vector<uint8_t> sha256(const std::string& data) {
    ...
    uint64_t bitLength = data.length() * 8;
    size_t paddedLength = ((data.length() + 8) / 64 + 1) * 64;
    std::vector<uint8_t> paddedData(paddedLength, 0);
    memcpy(&paddedData[0], data.c_str(), data.length());
...
int main() {
    ...
    std::string bookOfMarkText = download_text(url);

    std::vector<uint8_t> hash = sha256(bookOfMarkText);
